...
import { createHistory, iterateHistory, restore, restoreWithRewind } from "../src/diff";
...
test("Ensure diff history is smaller than classic history", () => {
    ...
    while(records.length > 0 && history.length > 0) {
        ...
        const [restored, futureRecord] = restoreWithRewind(state, history.pop() as Delta);
        future.push(futureRecord);
