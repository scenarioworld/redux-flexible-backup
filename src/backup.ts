 ...
 */
export type BackupLoadFunction<S, Stored> = (state: Stored|undefined) => S|undefined;
...
type UndefinedSaveGuard<M> = M extends undefined ? { save: () => undefined } : M;
...
export type StoredState<S, BackupInterface extends StateBackupInterface<S>> = {
    [K in keyof BackupInterface]: ReturnType<UndefinedSaveGuard<BackupInterface[K]>["save"]>;
}
...
export function createBackup
    <S extends StateOrSlice, BackupInterface extends StateBackupInterface<S>>
    (state: S, backupInterface: BackupInterface
...
export function loadBackup
    <S extends StateOrSlice, BackupInterface extends StateBackupInterface<S>>
    (state: Partial<S>, backupInterface: BackupInterface, store: StoredState<S, BackupInterface>
): Partial<S> {
